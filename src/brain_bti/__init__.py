...
def 낮은_초반_집중_유형(gen, name):
    ...
    gen.addText(
        "초반 10분동안의 두뇌 가동률 변화 추이는 대상이 집중함에 있어 도입부에", (x, y - step * 1), font_size=9
    )
    gen.addText(
        f"어떤 자세를 취하는지를 유추 할 수 있습니다. {name}님의 초반 두뇌 가동률 ", (x, y - step * 2), font_size=9
    )
    gen.addText(
        "변화 추이는 하락을 하는 경향을 보이며, 이는 초반에는 주어진 과제를 분석", (x, y - step * 3), font_size=9
    )
...
def 높은_초반_집중_유형(gen, name):
    ...
    gen.addText(
        "변화 추이는 상승을 하는 경향을 보이며, 이는 초반부터 강하게 집중력을 ", (x, y - step * 3), font_size=9
    )
...
def 낮은_중반_집중_유형(gen, name):
    ...
    gen.addText(
        "중반 10분은 실행된 시간에 관계 없이 초반 10분과 후반 10분을 제외한",
        (x + coef_x, y - step * 1),
        font_size=9,
    )
    gen.addText(
        "나머지 시간을 10분으로 요약해 나타냅니다. 즉, 중반 10분간의", (x + coef_x, y - step * 2), font_size=9
    )
    gen.addText(
        f"두뇌 가동률 변화 추이는 대상의 전체적인 가동률을 대변합니다. {name}님의",
        (x + coef_x, y - step * 3),
        font_size=9,
    )
    gen.addText(
        "중반 두뇌 가동률 변화 추이는 하락을 하는 경향을 보이며, 이는 지속성을",
        (x + coef_x, y - step * 4),
        font_size=9,
    )
...
def 높은_중반_집중_유형(gen, name):
    ...
    gen.addText(
        "중반 두뇌 가동률 변화 추이는 상승을 하는 경향을 보이며, 이는 집중력의",
        (x + coef_x, y - step * 4),
        font_size=9,
    )
...
def 낮은_후반_집중_유형(gen, name):
    ...
    gen.addText(
        "후반 10분동안의 두뇌 가동률 변화 추이는 대상이 집중함에 있어 공부",
        (x, y + coef_y - step * 1),
        font_size=9,
    )
    gen.addText(
        f"마무리 단계에서 어떤 자세를 취하는지를 유추 할 수 있습니다.  {name}님의 ",
        (x, y + coef_y - step * 2),
        font_size=9,
    )
    gen.addText(
        "후반 두뇌 가동률 변화 추이는 하락을 하는 경향을 보이며, 이는 본인의",
        (x, y + coef_y - step * 3),
        font_size=9,
    )
...
def 높은_후반_집중_유형(gen, name):
    ...
    gen.addText(
        "후반 두뇌 가동률 변화 추이는 상승을 하는 경향을 보이며, 이는 높은",
        (x, y + coef_y - step * 3),
        font_size=9,
    )
...
def 낮은_두뇌_가동률_변화_유형(gen, name):
    ...
    gen.addText(
        "변화량은 두뇌 가동률 변화 추이 표준 편차라고도 표현합니다. 가동률 ",
        (x + coef_x, y + coef_y - step * 1),
        font_size=9,
    )
    gen.addText(
        "변화량은 각각의 사용마다 일정한 가동률이 발현되었는지를 알려주는",
        (x + coef_x, y + coef_y - step * 2),
        font_size=9,
    )
    gen.addText(
        f"지표입니다. {name}님의 변화량은 낮음으로, 일정한 난도의 상황에서",
        (x + coef_x, y + coef_y - step * 3),
        font_size=9,
    )
...
def 높은_두뇌_가동률_변화_유형(gen, name):
    ...
    gen.addText(
        f"지표입니다. {name}님의 변화량은 높음으로, 다양한 난도의 상황에서",
        (x + coef_x, y + coef_y - step * 3),
        font_size=9,
    )
    gen.addText("집중을 하였습니다.", (x + coef_x, y + coef_y - step * 4), font_size=9)
