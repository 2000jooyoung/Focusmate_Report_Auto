...
def generate_두뇌가동률(df, name, date):
    ...
    for idx, week in enumerate(weeks):
        gen.addGraphics(
            (일별두되가동률_x + 일별두되가동률_step * idx, 일별두되가동률_y),
            f"{name}/일별두되가동률_{week}.png",
            scale=일별두되가동률_scale,
        )
        gen.addGraphics(
            (일별두되가동량_x + 일별두되가동량_step * idx, 일별두되가동량_y),
            f"{name}/일별두뇌가동량_{week}.png",
            scale=일별두되가동량_scale,
        )
    ...
    gen.addText(f"할 때 나타납니다. {name} 님의 주간 평균 두뇌가동률이 매우 높은 상태로", (20, 303), font_size=9)
    gen.addText(
