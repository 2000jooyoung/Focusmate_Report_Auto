...
def generate_두뇌가동률변화추이(df, name):
    ...
    plt.fill_between(range(len(average_values)), average_values - std_dev_values, average_values + std_dev_values, alpha=0.3, label='Standard Deviation', color="#9494FF")
    ...
    plt.savefig(f'{name}/두뇌가동률변화추이.png', transparent=True)
    make_color_transparent(f"{name}/두뇌가동률변화추이.png", f"{name}/두뇌가동률변화추이.png", "#FFFFFF")
    resize_image(f"{name}/두뇌가동률변화추이.png", f"{name}/두뇌가동률변화추이.png", (2500, 800))
            
        
        
color_dict = {1:'#6266EA', 2:'#9494FF', 3:'#C084EF', 4:'#E2BEFF'}
text_dict = {1:('R(안정형)', 'D(속도형)'), 2:('I(변동형)', 'E(유지형)'), 3:('A(차분형)', 'L(뒷심형)'), 4:('S(지속형)', 'V(변화형)')}
...
def build_bbti_graph(slope, number):
    ...
    background = alt.Chart(source).mark_bar(
        ...
        x=alt.X('background').axis(None),
        y=alt.X('label').axis(None)
    )#properties

    value_chart = alt.Chart(source).mark_bar(
        cornerRadius=7,
        color=color,
        ).encode(
...
def build_text(slope, number):
    color = [color_dict[number], '#505265', "#00000000"] if slope < 0 else ['#505265', color_dict[number], "#00000000"]
    source = pd.DataFrame({
        'x': [0, 65, 100],
        'y': [1, 1, 1],
...
def plot_and_save_뇌BTI결과(results, name):
    for idx, result in enumerate(results):
        ...
        first_chart.save(f"{name}/{idx_2_text_order[idx+1]}.png", format='png', scale_factor=2, background='transparent')
        make_color_transparent(f"{name}/{idx_2_text_order[idx+1]}.png", f"{name}/{idx_2_text_order[idx+1]}.png", "#FFFFFF")

        first_text = build_text(result, idx + 1)

        first_text.save(f"{name}/{idx_2_text_order[idx+1]}_text.png", format='png', scale_factor=8, background='transparent')
        make_color_transparent(f"{name}/{idx_2_text_order[idx+1]}_text.png", f"{name}/{idx_2_text_order[idx+1]}_text.png", "#FFFFFF")
