...
def text_for_h(gen, name):
    ...
    gen.addText("난이도의 과제를 수행했다는 것을 의미합니다. 높은 두뇌가동률이 ", (x, y - coef_y * 1), font_size=font_size)
...
def generate_뉴로프로필(df, name, date):
    neuroprofile = generate_뉴로프로필_list(df, name)
    ...
    # 일별 총 공부시간
    gen.addGraphics((80, 450), f"{name}/일별총공부시간.png", scale=0.5)

    # 두뇌가동률

    gen.addGraphics((350, 400), f"{name}/평균두뇌활동비율.png", scale=0.3)
    ...
    # 공부규칙성
    gen.addGraphics((70, 214), f"{name}/공부규칙성.png", scale=0.2)

    # 집중유형
    gen.addGraphics((355, 220), f"{name}/집중유형.png", scale=0.2)
