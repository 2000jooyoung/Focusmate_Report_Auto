...
def generate_공부시간(df, name, date):
    ...
    평균_주간_공부시간 = datetime(2000, 3, 17, 0) + timedelta(
        seconds=np.mean(df_without_zeros.time)
    )
    최대_주간_공부시간 = datetime(2000, 3, 17, 0) + timedelta(
        seconds=np.max(df_without_zeros.time)
    )
    최소_주간_공부시간 = datetime(2000, 3, 17, 0) + timedelta(
    ...
    if 평균_주간_공부시간.hour == 0:
        ...
        gen.addText(
            f"{평균_주간_공부시간.hour}시간 {평균_주간_공부시간.minute}분", (485, 690), font_size=10
        )

    gen.addText(
        f"{name} 님의 주간 평균 공부 시간은 {str(평균_주간_공부시간.hour) + '시간' if 평균_주간_공부시간.hour != 0 else ''} {평균_주간_공부시간.minute} 분입니다. 최단 시간은 {str(최소_주간_공부시간.hour) + '시간' if 최소_주간_공부시간.hour != 0 else ''}",
        (20, 330),
        font_size=9,
    )
    gen.addText(
        f"{최소_주간_공부시간.minute}분이었으며, 최장 공부 시간은 {str(최대_주간_공부시간.hour) + '시간' if 최대_주간_공부시간.hour != 0 else ''} {최대_주간_공부시간.minute}분이었습니다.",
        (20, 318),
        font_size=9,
    )
