...
def plot_and_save_일별총공부시간(df, name):
    slope = get_일별총공부시간_proportion(df)
    value = slope
    ...
    chart.save(f"{name}/일별총공부시간.png")
    make_color_transparent(f"{name}/일별총공부시간.png", f"{name}/일별총공부시간.png", "#FFFFFF")
    if slope >= 50:
        return "e"
    else:
        return "i"
    
def plot_and_save_공부규칙성(df, name):
    ...
    plt.savefig(f"{name}/공부규칙성.png")
    make_color_transparent(f"{name}/공부규칙성.png", f"{name}/공부규칙성.png", "#FFFFFF")
...
def plot_and_save_집중유형(df, name):
    custom_font_path = 'fonts/Noto_Sans_KR/static/NotoSansKR-Light.ttf'
    ...
    plt.ylabel("두뇌가동률 추이", fontproperties=custom_font, fontsize=20, labelpad=10)


    fig.savefig(f"{name}/집중유형.png")
    make_color_transparent(f"{name}/집중유형.png", f"{name}/집중유형.png", "#FFFFFF")
...
def generate_뉴로프로필_list(df, name):
    뇌BTI = ["i", "h", "c", "b"]

    뇌BTI[0] = plot_and_save_일별총공부시간(df, name)
    뇌BTI[1] = plot_and_save_평균두뇌활동비율(df, name)
    뇌BTI[2] = plot_and_save_공부규칙성(df, name)
    뇌BTI[3] = plot_and_save_집중유형(df, name)
