...
def get_start_end_scaled_range(df):
    ...
    for _, row in df.iterrows():
        start_ranges.append((row["startedAt"].hour * 60 +  row["startedAt"].minute) / (24 * 60) * 100)
        end_ranges.append((row["endAt"].hour * 60 +  row["endAt"].minute) / (24 * 60) * 100)
...
def generate_공부규칙성1(df, name):
    ...
    for start, end, week in zip(start_ranges, end_ranges, weeks):
        ...
        공부규칙성 = alt.Chart(source).mark_bar(
        ...
        공부규칙성 = 공부규칙성.configure_view(stroke='transparent')
        공부규칙성.save(f"{name}/공부규칙성_{week}.png", scale_factor=1)
        make_color_transparent(f"{name}/공부규칙성_{week}.png", f"{name}/공부규칙성_{week}.png", "#FFFFFF")
...
def save_공부_규칙성_2(data, name):
    ...
    for i, j in zip([i for i in range(4, 24, 2)] + [0, 2], [i for i in range(0, 24, 2)]):
    ...
    plt.savefig(f'{name}/공부규칙성2.png', transparent=True)
    make_color_transparent(f"{name}/공부규칙성2.png", f"{name}/공부규칙성2.png", "#FFFFFF")
    resize_image(f'{name}/공부규칙성2.png', f'{name}/공부규칙성2.png', (2000, 2000))


def generate_공부규칙성2(df, name):
    df['timestamp'] = pd.to_datetime(df['startedAt'])
    counts_list = [0] * 24

    for _, row in df.iterrows():
        if not pd.isna(row['timestamp']):
            interval_index = int((row['timestamp'].hour * 60 + row['timestamp'].minute) / 60)
