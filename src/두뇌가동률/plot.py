...
def generate_일별두뇌가동률(df, name):
    ...
    for i, week in enumerate(weeks):
        ...
        일별두되가동률 = alt.Chart(source).mark_bar(width=40, cornerRadiusTopLeft=5, cornerRadiusTopRight=5, height=20,).encode(
            x=alt.X('date', axis=None),
            y=alt.Y('sum(data)', axis=None),
            color=alt.Color('color', scale=alt.Scale(range=custom_colors), legend=None)
        )

        일별두되가동률 = 일별두되가동률.configure_view(stroke='transparent')
        일별두되가동률.save(f"{name}/일별두되가동률_{week}.png", scale_factor=1)
        make_color_transparent(f"{name}/일별두되가동률_{week}.png", f"{name}/일별두되가동률_{week}.png", "#FFFFFF")
...
def generate_일별두뇌가동량(df, name):   
    ...
    boa_df = pd.DataFrame({'날짜' : days, '가동량' : boas})
    ...
    boa_df['Day'] = pd.Categorical(boa_df['날짜'], categories=list(df.weekday), ordered=True)
    df_sorted = boa_df.copy()
    df_sorted.sort_values(by='Day', key=lambda x: x.map({day: i for i, day in enumerate(df.weekday)}), inplace=True)
    ...
    for week, data in zip(weeks, df_sorted["plot_가동량"]):
        ...
        일별두뇌가동량 = alt.Chart(source).mark_bar(
        ...
        make_color_transparent(f"{name}/일별두뇌가동량_{week}.png", f"{name}/일별두뇌가동량_{week}.png", "#FFFFFF")
...
def generate_일별두뇌가동량_text(axis_max_boa, axis_middle_boa, name):
    ...
    make_color_transparent(f"{name}/일별두뇌가동량_text_max.png", f"{name}/일별두뇌가동량_text_max.png", "#FFFFFF")
    
    source = pd.DataFrame({
        'x': [0],
        'y': [0],
        'value_text': [axis_middle_boa],
        'color' :["#9697A4"]
    })
    ...
    make_color_transparent(f"{name}/일별두뇌가동량_text_middle.png", f"{name}/일별두뇌가동량_text_middle.png", "#FFFFFF")
