...
def draw_I(gen):
    gen.addText("I(Imbalance)는 변동형입니다. 변동형의 사람들은 공부 중반에 두뇌 가동률이 ",(20,270), font_size=9)
...
def generate_두뇌가동유형(df, name, date):
    ...
    gen.addGraphics((공부규칙성_x + 공부규칙성_step * 0, 공부규칙성_y), f"{name}/두뇌가동률변화추이.png", scale=공부규칙성_scale) # 일

    mbti_x, mbti_y = 300, 160
    mbti_step = 50
    mbti_scale = 0.4

    gen.addGraphics((mbti_x, mbti_y + mbti_step * 3), f"{name}/폭발.png", scale=mbti_scale)
    gen.addGraphics((mbti_x, mbti_y + mbti_step * 2), f"{name}/유지.png", scale=mbti_scale)
    gen.addGraphics((mbti_x, mbti_y + mbti_step * 1), f"{name}/결정.png", scale=mbti_scale)
    gen.addGraphics((mbti_x, mbti_y + mbti_step * 0), f"{name}/변화.png", scale=mbti_scale)

    mbti_x, mbti_y = 305, -150
    mbti_step = 50
    mbti_scale = 0.125

    gen.addGraphics((mbti_x, mbti_y + mbti_step * 3), f"{name}/폭발_text.png", scale=mbti_scale)
    gen.addGraphics((mbti_x, mbti_y + mbti_step * 2), f"{name}/유지_text.png", scale=mbti_scale)
    gen.addGraphics((mbti_x, mbti_y + mbti_step * 1), f"{name}/결정_text.png", scale=mbti_scale)
    gen.addGraphics((mbti_x, mbti_y + mbti_step * 0), f"{name}/변화_text.png", scale=mbti_scale)
    ...
    gen.drawText(f"{name}님의 두뇌 유형은 {''.join(mbti).upper()}형입니다.",(20,325), font_size=10)
